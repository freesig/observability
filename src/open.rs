...
pub struct Context(api::Context);
...
macro_rules! span_context {
    ($span:expr, $lvl:expr) => {{
        let level = $crate::tracing::level_filters::LevelFilter::current().into_level();
        if let Some(level) = level {
            if level >= $lvl && !$span.is_disabled() {
                ...
                if let Some(span_ref) = registry.span(id) {
                    ...
                    if let Some(sb) = extensions.get_mut::<api::SpanBuilder>() {
                        let mut new_links = new_links
                            .into_iter()
                            .rev()
                            .take_while(|link| Some(link.span_context().span_id()) != sb.span_id)
                            .collect::<Vec<_>>();
                            new_links.reverse();
